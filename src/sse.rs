...
pub async fn torrent_events(
    ...
    let initial = match state.latest_torrents().await {
        Some(torrents) => {
            ...
            // OOB: Wrap list separately, counts already have their own OOB attributes
            let oob_html = format!("<div id=\"torrent-list\" hx-swap-oob=\"morph\">{}</div>{}", list_html, counts_html);
