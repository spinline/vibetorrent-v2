...
use crate::templates::{
    AddTorrentModalTemplate, IndexTemplate, StatsTemplate, TorrentRowTemplate, TorrentView,
};
...
pub async fn index(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse> {
    ...
    let downloading_count = torrents
        .iter()
        .filter(|t| t.state == TorrentState::Downloading)
        .count();
    let seeding_count = torrents
        .iter()
...
pub async fn torrents_filtered(
    ...
    let all_torrents = state.rtorrent.get_torrents().await.unwrap_or_default();
    let html = torrents_service::render_torrents_html(
        &state,
        &query,
        Some(filter.as_str()),
        &all_torrents,
...
pub async fn stats_partial(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse> {
    let stats = state
        .rtorrent
        .get_global_stats()
        .await
